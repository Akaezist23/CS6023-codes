                                ...
                                int H, int W)
{
    ...
    // if (blockIdx.x >= H || threadIdx.x >= W) return;
    int idx = (blockIdx.x * blockDim.x + threadIdx.x); // launching with <<<H, W>>>, we need (i*W + j)
    int base = 3 * idx;  
    int r = base, g = base + 1, b = base + 2;
    float red = (float)d_rgb[r];
    float gre = (float)d_rgb[g];
    float blu = (float)d_rgb[b];
    d_gray[idx] = W_RED * red + W_GREEN * gre + W_BLUE * blu;
    // ==== END TODO 1 ====
}
                             ...
                             int Hr, int Wr)
{
    ...
    int ox = blockIdx.x, oy = threadIdx.x; //lauch using (Wr, Hr)
    float fy = oy * scaleY, fx = ox * scaleX;
    int y0 = floorf(fy), x0 = floorf(fx);
 ...
 *      offsetX = (Wr - Wc) / 2
 *
 *      out(y, x) = in(y + offsetY, x(H * W * 3)*sizeof(unsigned char) + offsetX)
 *
                           ...
                           int Hc, int Wc)
{
    ...
    int offy = (Hr - Hc)/2, offx = (Wr - Wc)/2;
    int x = blockIdx.x, y = threadIdx.x; //launch (Wc, Hc)
    int id1 = y * Wc + x, id2 = (int)((float)y + offy) * Wr + ((int)(float)x + offx);
    d_cropped[id1] = d_resized[id2]; 
                                ...
                                float mean, float stdv)
{
    ...
    float tmp = (float) d_cropped[idx] / 255;
    
    d_out[idx] = (tmp - mean)/ stdv;
    // ==== END TODO 4 ====
} 

__global__ void kernelPrint(const float *arr, int H, int W) {
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            int idx = i * W + j;
            printf("%f ", arr[idx]);
...
int main(int argc, char **argv)
{
    ...
    grayscaleKernel<<<H, W>>>(d_rgb, d_gray, H, W);
    resizeKernel<<<Wr, Hr>>>(d_gray, d_resized, H, W, Hr, Wr);
    cropKernel<<<Wc, Hc>>>(d_resized, d_cropped, Hr, Wr, Hc, Wc);
    normalizeKernel<<<Hc, Wc>>> (d_cropped, d_out, Hc, Wc, mean, stdv);  
